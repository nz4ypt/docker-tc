...
DUMPFILE=infodba_tc124_ootb.dmp
source oraenv
...
PROMPT CREATE SERVER MANAGER INSTANCE;
CREATE TABLESPACE TCCLUSTER DATAFILE '/opt/oracle/oradata/$ORACLE_SID/$ORACLE_PDB/TCCLUSTER.dat' SIZE 10M AUTOEXTEND ON MAXSIZE 200M EXTENT MANAGEMENT LOCAL UNIFORM SIZE 160K;
CREATE USER TCCLUSTER IDENTIFIED BY tccluster DEFAULT TABLESPACE TCCLUSTER TEMPORARY TABLESPACE temp QUOTA 200M on TCCLUSTER;
GRANT CONNECT TO TCCLUSTER;
GRANT CREATE TABLE TO TCCLUSTER;
exit;
EOF
...
UPDATE PIMANVOLUME SET PNODE_NAME='corp'; 
COMMIT; 
exit;
EOF
